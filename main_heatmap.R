...
# HM <- readRDS("HM_BEL_leaky.RData")
# HM <- readRDS("HM_BEL_leaky_u_constant.RData")
# HM <- readRDS("HM_BEL_aorn.RData")
# HM <- readRDS("HM_BEL_aorn_u_constant.RData")
# param <- "ve"

# HM <- readRDS("HM_BEL_rollout_speed.RData")
# param <- "rollout"

# HM <- readRDS("HM_countries_90_aorn.RData")
# param <- "country"
 
HM <- readRDS("HM_R0.RData")
param <- "R0"
...
pS2_I <- plot_heatmap(reshape2::melt(S2_Inew), num, param)  +
  ylab("Cumulative\nincidence\n\n") + 
  ggtitle("Continuous rollout (1% per day)") +
  onlyy_theme 
...
# export as 6x6
g <- ggarrange(pS2_I, pS3_I, pS2_M, pS3_M, pS2_Y, pS3_Y,
          bottom = textGrob("Total vaccine supply (% of population)", gp = gpar(fontsize = 12)),
          padding = unit(0.5, "line"))
