...
# _____________________________________________________________________
# FIGURE 2: Reduction in infections & deaths ####
# Scenario 1: R0 = 1.3, continuous rollout at num_perday = 0.2%/day
# Scenario 2: R0 = 2.6, continuous rollout at num_perday = 0.2%/day
# _____________________________________________________________________
ptm <- proc.time()
...
# export as pdf 5x4"
panel <- ggarrange(infect_1, infect_2, mort_1, mort_2, 
                   nrow = 2,
                   labels = c('A', 'B', 'C', 'D'),
...
this_scale <- scale_u_for_R0(u_var, C, 2.6) # so that realized R is 1.5
...
for (i in seq(0, 50, by = 1)){
  j <- i/100
  list_all[[paste0(i)]] <- run_sim_new(this_C, j, "all", num_perday, v_e_type,
                                       this_v_e, u = u_var, syn_sero_compartments = IC_syn)
  list_kids[[paste0(i)]] <- run_sim_new(this_C, j, "kids", num_perday, v_e_type,
                                        this_v_e, u = u_var, syn_sero_compartments = IC_syn)
  list_adults[[paste0(i)]] <- run_sim_new(this_C, j, "adults", num_perday, v_e_type,
                                          this_v_e, u = u_var,syn_sero_compartments = IC_syn)
  list_elderly[[paste0(i)]] <- run_sim_new(this_C, j, "elderly", num_perday, v_e_type,
                                           this_v_e, u = u_var,syn_sero_compartments = IC_syn)
  list_twentyplus[[paste0(i)]] <- run_sim_new(this_C, j, "twentyplus", num_perday, v_e_type,
                                              this_v_e, u = u_var, syn_sero_compartments = IC_syn)
  list_all_var[[paste0(i)]] <- run_sim_new(this_C, j, "all", num_perday, v_e_type,
                                           this_v_e, u = u_var,syn_sero_compartments = IC_syn, sp = this_sp, se = this_se)
  list_kids_var[[paste0(i)]] <- run_sim_new(this_C, j, "kids", num_perday, v_e_type, 
                                            this_v_e, u = u_var,syn_sero_compartments = IC_syn, sp = this_sp, se = this_se)
  list_adults_var[[paste0(i)]] <- run_sim_new(this_C, j, "adults", num_perday, v_e_type, 
                                              this_v_e, u = u_var,syn_sero_compartments = IC_syn, sp = this_sp, se = this_se)
  list_elderly_var[[paste0(i)]] <- run_sim_new(this_C, j, "elderly", num_perday, v_e_type,
                                               this_v_e, u = u_var, syn_sero_compartments = IC_syn, sp = this_sp, se = this_se)
  list_twentyplus_var[[paste0(i)]] <- run_sim_new(this_C, j, "twentyplus", num_perday, v_e_type,
                                                  this_v_e, u = u_var,syn_sero_compartments = IC_syn, sp = this_sp, se = this_se)
}
proc.time() - ptm
p_mort <- plot_over_vax_avail("deaths", TRUE)
