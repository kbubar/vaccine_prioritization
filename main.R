...
# vaccine efficacy
# p is the final v_e % (as a decimal)
...
# * Simple model (age-dependent contact structure, IFR & susceptibility) ----
...
# run model using seroprevalence uncertainty from Larremore 2020
...
# 750*600
outcome <- "deaths"
pB <-  plot_serotesting_over_vax_avail(outcome)

outcome = "cases"
pD <-  plot_serotesting_over_vax_avail(outcome)

# BEL sero
grid.arrange(arrangeGrob(pA,  left = textGrob("Reduction in deaths (%)", rot = 90, vjust = 0.5, gp = gpar(fontsize = 22))),
             arrangeGrob(pB, left = textGrob("")),  
             arrangeGrob(pC, left = textGrob("Reduction in infections (%)", rot = 90, vjust = 0.5, gp = gpar(fontsize = 22))),
             arrangeGrob(pD, left = textGrob("")),
             ncol=2, widths=c(2.6, 2.3),
             heights=c(2.3, 2.6),
             bottom = textGrob("Total vaccine supply (% of pop)", vjust = 0, gp = gpar(fontsize = 22)))

# NY sero
grid.arrange(pB, pD, ncol = 2)


# * Plot I and R curves for all ages over time----
compartment <- "I"
N <- N_i

# NAS figs
p1 <- plot_allages_onestrategy(list_elderly[[1]], "I")
print(p1)

p2 <- plot_allages_onestrategy(list_elderly[[21]], "I")
print(p2)

legend <- get_legend(p1)

p2 <- plot_allages_onestrategy(list_all[[2]], "I") +
  theme(legend.position = "none") +
  ggtitle("all, 1% Vax")

p3 <- plot_allages_onestrategy(list_all[[26]], "I") +
  theme(legend.position = "none") +
  ggtitle("all, 25% Vax")

p4 <- plot_allages_onestrategy(list_kids[[2]], "I") +
  theme(legend.position = "none") +
  ggtitle("Kids, 1% Vax")

p5 <- plot_allages_onestrategy(list_kids[[26]], "I") +
  theme(legend.position = "none") +
  ggtitle("Kids, 25% Vax")

p6 <- plot_allages_onestrategy(list_adults[[2]], "I") +
  theme(legend.position = "none") +
  ggtitle("Adults, 1% Vax")

p7 <- plot_allages_onestrategy(list_adults[[26]], "I") +
  theme(legend.position = "none") +
  ggtitle("Adults, 25% Vax")
...
#export as 800 * 900
ggplot(df, aes(x = groups, y = v_e_plot*100)) +
  geom_step(size = 1.5) +
...
# TODO: old plotting code, t needs to be updated
# i <- 1
...
# TODO: old plotting code, needs to be updated
# t <- seq(0,400,1) 
# 
# percent_vax <- 0.10
