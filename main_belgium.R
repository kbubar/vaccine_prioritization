...
C <- readRDS(paste0("C_", country, "_bytens_all.RData"))
...
grid.arrange(arrangeGrob(p3,
                         ...
                         ncol=1, 
                         widths=c(2.3), 
                         left = textGrob("Age-distribution of vaccines", rot = 90, vjust = 1, gp = gpar(fontsize = 18)), 
                         bottom = textGrob("Age (years)", vjust = 0, gp = gpar(fontsize = 18))))
...
# * Other Plots ----
# # Plot difference between serology testing and no testing
# # start by changing plot_over_vax_avail to output reduction_in_cases/deaths
# reduc_cases_belgium_notest <- plot_over_vax_avail(outcome, "Serology (Belgium), no testing", list_all_sero_notest[[1]], list_all_sero_notest, list_kids_sero_notest, list_adults_sero_notest, list_elderly_sero_notest, list_twentyplus_sero_notest)
# reduc_cases_belgium_test <- plot_over_vax_avail(outcome, "Serology (25%), no testing", list_all_sero_test[[1]], list_all_sero_test, list_kids_sero_test, list_adults_sero_test, list_elderly_sero_test, list_twentyplus_sero_test)
# reduc_deaths_belgium_notest <- plot_over_vax_avail(outcome, "Serology (25%), no testing", list_all_sero_notest[[1]], list_all_sero_notest, list_kids_sero_notest, list_adults_sero_notest, list_elderly_sero_notest, list_twentyplus_sero_notest)
# reduc_deaths_belgium_test <- plot_over_vax_avail(outcome, "Serology (25%), no testing", list_all_sero_test[[1]], list_all_sero_test, list_kids_sero_test, list_adults_sero_test, list_elderly_sero_test, list_twentyplus_sero_test)
# 
# num_strategies <- 5
...
#            rep("Twenty +", num_per_list*2))
# 
# temp <-  c(rep("constant", num_per_list), rep("var", num_per_list))
# variable <- c(rep(temp, num_strategies))
# 
# dif <- reduc_cases_50_test - reduc_cases_50_notest
# df <- data.frame(vax_avail, strat, dif, variable)
# 
# theme_set(theme_minimal(base_size = 20))
# 
# ggplot(df[df$variable == "var", ], aes(x = vax_avail, y = dif, col = strat)) +
#   geom_line(size = 2, alpha = 0.9) +
#   xlab("Vaccine available (% of total pop)") +
#   ylab("Reduction in cases from using serology (%)") +
#   #ggtitle("Benefit from using serology test, IC 50%") +
#   scale_color_brewer(palette = "Dark2", name = "Allocation Strategy",
#                      labels =  c("Young Adults", "All", "Elderly", "Kids",
#                                  "Adults")) +
#   #theme(legend.position = "none") +
#   ylim(0, 15) + 
#   xlim(0, 50) + 
